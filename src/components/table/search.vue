		...
		<el-form ref="tableSearchRef" :model="state.form" size="default" label-width="100px" class="table-form">
			<div class="mb10 mr10" v-for="(val, key) in search" :key="key" v-show="key < showNumber || state.isToggle">
				<template v-if="val.type !== ''">
					<!-- input -->
					<el-input v-model="state.form[val.prop]" :placeholder="val.placeholder" clearable v-if="val.type === 'input'" style="width: 180px" />
					<!-- date -->
					<el-date-picker
...
// 定义子组件向父组件传值/事件
const emit = defineEmits(['selectChange', 'reset']);
