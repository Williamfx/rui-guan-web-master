	...
	<div class="layout-navbars-breadcrumb">
		...
		<el-breadcrumb class="layout-navbars-breadcrumb-hide">
			<transition-group name="breadcrumb">
				<el-breadcrumb-item v-for="(v, k) in state.breadcrumbList" :key="v">
...
const initRouteSplit = async (toRoute: any) => {
	...
	await getBreadcrumbList(routesList.value);
	if (toRoute.name === 'home' || (toRoute.name === 'notFound' && state.breadcrumbList.length > 1)) state.breadcrumbList.shift();
	if (state.breadcrumbList.length > 0) state.breadcrumbList[state.breadcrumbList.length - 1].meta.tagsViewName = toRoute.meta.title;
};
// 页面加载时
onMounted(() => {
